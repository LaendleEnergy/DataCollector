...
CREATE TABLE public.measurement (
                                    ...
                                    meter_device_id varchar(255) NOT NULL,
                                    CONSTRAINT measurement_pkey PRIMARY KEY (reading_time, meter_device_id)
);
...
CREATE UNIQUE INDEX idx_timestamp_device_id ON public.measurement USING btree (meter_device_id, reading_time);
CREATE INDEX measurement_reading_time_idx ON public.measurement USING btree (reading_time DESC);
SELECT create_hypertable('measurement', 'reading_time',  partitioning_column => 'meter_device_id', number_partitions => 10);
...
CREATE TABLE public.tag (
                            reading_timerange tsrange,
                            devicecategory_category_name varchar(255) NOT NULL,
                            meter_device_id  text NOT NULL,
                            device_name text NOT NULL,
                            CONSTRAINT tag_pkey PRIMARY KEY (reading_timerange, devicecategory_category_name, meter_device_id, device_name),
                            CONSTRAINT tag_exclusion_constraint EXCLUDE USING gist (
                                reading_timerange WITH &&,
                            ...
                            CONSTRAINT fk_device FOREIGN KEY (device_name, meter_device_id) REFERENCES public.device(device_name, meter_device_id)
);
...
CREATE TABLE public.averagepriceperwh (
    ...
    PRIMARY KEY (meter_device_id, start_date)
);

CREATE TYPE TAG_RECORD as (devicecategory_category_name text, device_name text, reading_timerange tsrange);

CREATE OR REPLACE VIEW measurement_w_t AS
(
SELECT m.meter_device_id, m.reading_time,
       ...
       NULLIF(ARRAY_AGG((t.devicecategory_category_name, t.device_name, t.reading_timerange)::TAG_RECORD), array['(,,)'::tag_record]) as tags
FROM measurement m
         LEFT OUTER JOIN TAG t
                         ON m.meter_device_id = t.meter_device_id
                             AND t.reading_timerange @> m.reading_time
GROUP BY m.meter_device_id, m.reading_time
    );

CREATE OR REPLACE VIEW tag_view AS
(
SELECT LOWER(reading_timerange) as time_start, UPPER(reading_timerange) as time_end,
       meter_device_id, device_name, devicecategory_category_name
FROM tag
    );
...
'
    DECLARE
        t TAG_RECORD;
    BEGIN
        INSERT INTO measurement(reading_time, meter_device_id, current_l1a, current_l2a, current_l3a,
        ...
        SELECT NEW.reading_time, NEW.meter_device_id, NEW.current_l1a, NEW.current_l2a, NEW.current_l3a,
...
'
    BEGIN
        INSERT INTO tag(reading_timerange, devicecategory_category_name, meter_device_id, device_name)
    ...
    EXCEPTION
        WHEN exclusion_violation THEN
            ...
            RETURNING *),
            insert_timespan AS (
                SELECT CASE WHEN MIN(LOWER(reading_timerange)) < NEW.time_start
                                THEN MIN(LOWER(reading_timerange)) ELSE NEW.time_start END AS time_start,
                       CASE WHEN MAX(UPPER(reading_timerange)) > NEW.time_end
                                THEN MAX(UPPER(reading_timerange)) ELSE NEW.time_end END AS time_end,
                      meter_device_id, device_name, devicecategory_category_name
                FROM deleted_rows
                GROUP BY meter_device_id, device_name, devicecategory_category_name
            )
            INSERT INTO tag(reading_timerange, devicecategory_category_name, meter_device_id, device_name)
            SELECT tsrange(time_start, time_end), devicecategory_category_name, meter_device_id, device_name
            FROM insert_timespan
            GROUP BY meter_device_id, device_name, devicecategory_category_name, time_start, time_end;
            RETURN NEW;
    END;
'
    LANGUAGE plpgsql;
...
'
    ...
    BEGIN
        ...
        SET
            ...
            meter_device_id = NEW.meter_device_id,
        ...
        WHERE reading_time = NEW.reading_time AND meter_device_id = NEW.meter_device_id;
        FOREACH t in ARRAY NEW.tags
            LOOP
                INSERT INTO tag (meter_device_id,  devicecategory_category_name, device_name, reading_timerange)
                VALUES(NEW.meter_device_id, t.devicecategory_category_name, t.device_name, t.reading_timerange)
                ON CONFLICT (meter_device_id, devicecategory_category_name, device_name, reading_timerange)
                    DO NOTHING;
            END LOOP;
        RETURN NEW;
    END;
